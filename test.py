...
from model1_location import LocationPredictor
from model2_template import TemplatePredictor
from model3_scale import ScalePredictor
from model4_deformation import DeformationPredictor
...
torch.manual_seed(11111111) # for reproducibility
...
fixed_location = True # set "True" for comparison against other methods "False" otherwise
fixed_template = False
fixed_scale = False

save_dpath = './test_semantic_maps'
# -----------------------------------------------------------------------------
...
for i in range(len(data)):
    ...
    for trial in range(trials_per_sample):
        ...
        all_images.save(os.path.join(save_root, f'{i+1}--{trial+1}.png'))
